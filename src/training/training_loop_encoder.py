...
def test(E, Gs, Inv, ph_portraits, ph_landmarks, ph_keypoints, training_mode, submit_config):
    with tf.name_scope("Run"), tf.control_dependencies(None):
        with tf.device("/cpu:0"):
            ...
            in_split_keypoints = tf.split(ph_keypoints, num_or_size_splits=submit_config.num_gpus, axis=0)
        ...
        for gpu in range(submit_config.num_gpus):
            with tf.device("/gpu:%d" % gpu):
                ...
                latent_w = E.get_output_for(embedded_w_tensor, in_landmarks_gpu, in_keypoints_gpu, phase=False)
...
def training_loop(
    ...
    with tf.name_scope('input'):
        ...
        placeholder_real_keypoints_train = tf.placeholder(tf.float32, [submit_config.batch_size, 136], name='placeholder_real_keypoints_train')
        ...
        placeholder_keypoints_shuffled_test = tf.placeholder(tf.float32, [submit_config.batch_size_test, 136], name='placeholder_keypoints_shuffled_test')
        ...
        real_split_keypoints = tf.split(placeholder_real_keypoints_train, num_or_size_splits=submit_config.num_gpus, axis=0)
        real_split_keypoints_shuffled = tf.split(placeholder_keypoints_shuffled_train, num_or_size_splits=submit_config.num_gpus, axis=0)
    ...
    for gpu in range(submit_config.num_gpus):
        ...
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            real_split_keypoints_gpu = process_reals(real_split_keypoints[gpu], mirror_augment, [0, 255], drange_net)
            real_split_keypoints_shuffled_gpu = process_reals(real_split_keypoints_shuffled[gpu], mirror_augment, [0, 255], drange_net)
            with tf.name_scope('E_loss'), tf.control_dependencies(None):
                E_loss, recon_loss, adv_loss = dnnlib.util.call_func_by_name(E=E_gpu, G=G_gpu, D=D_gpu, Inv=Inv_gpu, perceptual_model=perceptual_model, real_portraits=real_portraits_gpu, shuffled_portraits=shuffled_portraits_gpu, real_landmarks=real_landmarks_gpu, shuffled_landmarks=shuffled_landmarks_gpu, real_keypoints=real_split_keypoints_gpu, shuffled_keypoints=real_split_keypoints_shuffled_gpu, training_flag=placeholder_training_flag, **E_loss_args)
                E_loss_rec += recon_loss
                E_loss_adv += adv_loss
            with tf.name_scope('D_loss'), tf.control_dependencies(None):
                D_loss, loss_fake, loss_real, loss_gp = dnnlib.util.call_func_by_name(E=E_gpu, G=G_gpu, D=D_gpu, Inv=Inv_gpu, real_portraits=real_portraits_gpu, shuffled_portraits=shuffled_portraits_gpu, real_landmarks=real_landmarks_gpu, real_keypoints=real_split_keypoints_gpu, shuffled_keypoints=real_split_keypoints_shuffled_gpu, training_flag=placeholder_training_flag, **D_loss_args) # change signature in ...
    ...
    fake_X_val = test(E, Gs, Inv, placeholder_real_portraits_test, placeholder_real_landmarks_test, placeholder_real_keypoints_test, None, submit_config)
    ...
    for it in range(start, max_iters):
        batch_stacks, batch_keypoint = sess.run(stack_batch_train)
        batch_portraits = batch_stacks[:,0,:,:,:]
        batch_landmarks = batch_stacks[:,1,:,:,:]
        
        batch_stacks_secondary, batch_keypoint_secondary = sess.run(stack_batch_train_secondary)
        ...
        feed_dict_1 = {placeholder_real_portraits_train: batch_portraits, placeholder_real_landmarks_train: batch_landmarks, placeholder_real_shuffled_train:batch_shuffled, placeholder_landmarks_shuffled_train:batch_lm_shuffled, placeholder_training_flag: training_flag, placeholder_real_keypoints_train:batch_keypoint, placeholder_keypoints_shuffled_train = batch_keypoint_secondary}
        ...
        if it % 500 == 0:
            batch_stacks_test, batch_keypoint_test = sess.run(stack_batch_test)
            batch_portraits_test = batch_stacks_test[:,0,:,:,:]
            batch_landmarks_test = batch_stacks_test[:,1,:,:,:]
            
            batch_stacks_test_secondary, batch_keypoint_test_secondary = sess.run(stack_batch_test_secondary)
            ...
            batch_keypoint_test = misc.adjust_dynamic_range(batch_keypoint_test.astype(np.float32), [0, 255], [-1., 1.])
            batch_keypoint_test_secondary = misc.adjust_dynamic_range(batch_keypoint_test_secondary.astype(np.float32), [0, 255], [-1., 1.])
