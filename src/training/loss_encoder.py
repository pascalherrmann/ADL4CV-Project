...
def appearance_training(E, G, D, Inv, perceptual_model, real_portraits, shuffled_portraits, real_landmarks, shuffled_landmarks, real_keypoints, shuffled_keypoints, training_flag, feature_scale=0.00005, D_scale=0.1, perceptual_img_size=256):
    ...
    latent_w = E.get_output_for(embedded_w_tensor, real_landmarks, real_keypoints, phase=True)
...
def pose_training(E, G, D, Inv, perceptual_model, real_portraits, shuffled_portraits, real_landmarks, shuffled_landmarks, real_keypoints, shuffled_keypoints, training_flag, feature_scale=0.00005, D_scale=0.1, perceptual_img_size=256):
    '''
    # CYCLE CONSISTENCY
    ...
    * 5: + Take Reconsturction & Perceptual Loss between Reconstructed & Original Image
    ...
    embedded_w_tensor = tf.reshape(embedded_w, [real_portraits.shape[0], num_layers, latent_dim])
    # 1
    w_manipulated = E.get_output_for(embedded_w_tensor, shuffled_landmarks, shuffled_keypoints, phase=True)
    ...
    # 3
    w_reconstructed = E.get_output_for(w_manipulated_tensor, real_landmarks, real_keypoints, phase=True)
...
def E_loss(E, G, D, Inv, perceptual_model, real_portraits, shuffled_portraits, real_landmarks, shuffled_landmarks, real_keypoints, shuffled_keypoints, training_flag, feature_scale=0.00005, D_scale=0.1, perceptual_img_size=256):

    with tf.device("/cpu:0"):
        appearance_flag = tf.math.equal(training_flag, "appearance")

    loss, recon_loss, adv_loss = tf.cond(appearance_flag, lambda: appearance_training(E, G, D, Inv, perceptual_model, real_portraits, shuffled_portraits, real_landmarks, shuffled_landmarks, training_flag, feature_scale, D_scale, perceptual_img_size), lambda: pose_training(E, G, D, Inv, perceptual_model, real_portraits, shuffled_portraits, real_landmarks, shuffled_landmarks, real_keypoints, shuffled_keypoints, training_flag, feature_scale, D_scale, perceptual_img_size))

    return loss, recon_loss, adv_loss

#----------------------------------------------------------------------------
# Discriminator loss function.
def D_logistic_simplegp(E, G, D, Inv, real_portraits, shuffled_portraits, real_landmarks, real_keypoints, shuffled_keypoints, training_flag, r1_gamma=10.0):
